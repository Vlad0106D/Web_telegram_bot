...
try:
    from config import SIGNAL_MIN_CONF
except Exception:
    SIGNAL_MIN_CONF = 70
    ...
    try:
        ...
        for sym in favs:
            ...
            try:
                ...
                log.exception("Breaker failed for %s %s", sym_u, tf)

            # ---------- 2) STRATEGY ----------
            try:
                res = await analyze_symbol(sym_u, tf=tf)
                ...
                if signal in ("long", "short") and conf >= int(SIGNAL_MIN_CONF):
                    key_s = (res.get("symbol", sym_u).upper(), tf, signal)
                ...
                log.exception("Strategy analyze failed for %s %s", sym_u, tf)

            # ---------- 3) REVERSAL ----------
            try:
                rev_events = await detect_reversals(sym_u)
                ...
                log.exception("Reversal detection failed for %s", sym_u)

            # ---------- 4) FUSION (LAZY IMPORT, чтобы не валить бота) ----------
            try:
                if FUSION_ENABLED:
                    from services.fusion import analyze_fusion, format_fusion_message  # lazy import

                    fev = await analyze_fusion(sym_u, tf)
                    if fev:
            ...
            try:
                if FIBO_ENABLED and tf in FIBO_TFS:
                    fibo_events = await analyze_fibo(sym_u, tf)
...
def schedule_watcher_jobs(
    ...
    for tf in norm_tfs:
        ...
        created.append(name)
        log.info("Watcher job scheduled: name=%s, interval=%ss, chat_id=%s", name, interval_sec, default_chat)

    log.info("Watcher scheduled for TFs=%s, interval=%ss", ", ".join(norm_tfs) if norm_tfs else "[]", interval_sec)
    return created
...
async def cmd_watch_on(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"Порог Strategy: {SIGNAL_MIN_CONF}\n"
