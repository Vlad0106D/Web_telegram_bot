...
def run_once(*, candles_limit: int = 200) -> RunnerResult:
    ...
    One MM v2 cycle:
      ...
      4) for each newly written snapshot timestamp -> detect/store LIVE events
    """
    if not ping():
    ...
    for w in wrote:
        ...
        try:
            # 1) compute regime/phase for this new closed candle
            ...
            # 2) detect & store LIVE events (safe: uses UNIQUE(snapshot_id,event_type))
            try:
                from mm_v2.events import detect_and_store_events_for_snapshot
                detect_and_store_events_for_snapshot(
                    snapshot_id=sid,
                    ts=ts,
                    symbol=w.symbol,
                    tf=w.tf,
                ...
                log.exception("event detect failed for %s %s %s", w.symbol, w.tf, ts.isoformat())
